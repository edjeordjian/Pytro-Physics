...
from constants.permeability_constants import (PERMEABILITY_TIMUR, TIMUR_TRACK_NAME, CONSTANT_LBL, INVALID_CONSTANT,
                                              DEFAULT_TIMUR_CONSTANT, K_LBL, MD_LBL)

from constants.porosity_constants import EFFECTIVE_POROSITY_CURVE

from constants.swirr_constants import SWIRR_TAB_NAME

from services.permeability_service import get_timur_permeability
...
class PermeabilityTimur(QWidgetWithSections):
    def __init__(self):
        super().__init__(PERMEABILITY_TIMUR)

        self.init_ui(PERMEABILITY_TIMUR)
    ...
    def curves_section(self):
        ...
        self.constant_textbox = QLineEdit(DEFAULT_TIMUR_CONSTANT)
    ...
    def preview(self):
        ...
        self.curve_to_save = get_timur_permeability(porosity,
                                                    swirr,
                                                    float(constant))
        ...
        self.add_curve_with_y_label({
            'tab_name': PERMEABILITY_TIMUR,

            'track_name': TIMUR_TRACK_NAME,

            'curve_name': TIMUR_TRACK_NAME,
        ...
        graphic_window.draw_tracks(PERMEABILITY_TIMUR)
