...
from services.ipr_service import get_voguel
from services.tools.string_service import is_number


class VoguelTab(QWidgetStandAlone):
    def __init__(self):
        super().__init__("Voguel")

        self.initUI()

        self.numeric_inputs.extend([self.customMaxFlowQle, self.reservoirPressureQle])

        self.add_serializable_attributes(self.curve_selectors + [self.iprCurveSizeSlider, self.iprLineStyleCbo,
              self.iprColorCbo, self.customMaxFlowQle, self.reservoirPressureQle])
