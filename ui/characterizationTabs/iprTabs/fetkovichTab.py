...
from services.ipr_service import (get_fetkovich, get_h_from_curve, get_k_from_curve)
from services.tools.string_service import is_number


class FetkovichTab(QWidgetStandAlone):
    def __init__(self):
        super().__init__("Fetkovich")

        self.initUI()

        self.numeric_inputs.extend([self.customMinDepthQle, self.customMaxDepthQle,
              self.customMaxFlowQle, self.netPayQle, self.permeabilityQle, self.reservoirPressureQle,
              self.damageFactorQle, self.wellRadiusQle, self.reservoirRadiusQle,
              self.volumeFactorQle, self.viscosityQle, self.bubblePressureQle,
              self.nExponentQle])
        ...
        self.add_serializable_attributes(self.curve_selectors + [self.depthFullLasRb, self.depthCustomRb,
              ...
              self.bubblePressureQle, self.nExponentQle, self.distanceUnitMetersRb, self.distanceUnitFeetRb])
    ...
    def initUI(self):
        ...
        self.nExponentLbl = QLabel("Exponente n:")
        self.nExponentQle = QLineEdit()
        self.nExponentQle.setStyleSheet(QLE_NAME_STYLE)
        self.nExponentQle.setPlaceholderText("n")

        self.nExponentLayout = QHBoxLayout()
        self.nExponentLayout.addWidget(self.nExponentLbl)
        self.nExponentLayout.addWidget(self.nExponentQle, alignment=Qt.AlignmentFlag.AlignLeft)

        self.gridLayout.addLayout(self.nExponentLayout, row, 0, 1, 2)
    ...
    def checkErrorNumberInputs(self):
        ...
        if not is_number(self.nExponentQle.text()):
            AlertWindow("El valor de exponente 'n' ingresado es invalido, ingrese un numero (Se usa punto '.' como separador decimal)")
            return True
    ...
    def getIpr(self):
        ...
        config_aux = get_fetkovich(
            ...
            n = float(self.nExponentQle.text()),
