...
from services.crossplot_service import get_mineral_identification_1

from services.tools.pandas_service import set_nan_in_array_if_another_is_nan


class MineralIdentification1Tab(QWidgetStandAlone):
    def __init__(self):
    ...
    def preview(self):
        ...
        config_aux = get_mineral_identification_1()
        ...
        k_curve = self.well \
            .wellModel \
            .get_partial_ranged_df_curve(k_curve_name, minDepth, maxDepth, "", ""),

        th_curve = self.well \
            .wellModel \
            .get_partial_ranged_df_curve(th_curve_name, minDepth, maxDepth, "", ""),
        ...
        y_title = self.well.wellModel.get_label_for(th_curve_name, "Thorio")
        ...
        for i in range(len(config_aux['lines'])):
            config['line_groups'].append( {
                    'x_axis': config_aux['lines'][i]['k'],
                    'y_axis': config_aux['lines'][i]['th'],
                    'color': color_cbo.itemText(i),
                    'line': LINE_TYPE_CONSTANTS["SOLID_LINE"],
                    'line_name': config_aux['lines'][i]['name']
                } )
