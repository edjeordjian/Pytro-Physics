...
from services.crossplot_service import get_hingle
...
class HingleTab(QWidgetStandAlone):
    def __init__(self):
        super().__init__("Hingle")
        ...
        self.add_serializable_attributes(self.curve_selectors + [self.depthFullLasRb, self.depthCustomRb,
              ...
              self.forceZAxisColormapCbo, self.hingleColorCbo, self.hingleMarkerStyleCbo,
              self.aConstantQle, self.mConstantQle, self.nConstantQle, self.rwQle])
    ...
    def preview(self):
        ...
        config_aux = get_hingle(self.well.wellModel.get_partial_ranged_df_curve(self.phieCbo.currentText(), minDepth, maxDepth, "", ""),
                                rw,
                                a,
                                m,
                                n
        ...
        rt_1m_values = self.well.wellModel.get_partial_ranged_df_curve(self.rtCbo.currentText(), minDepth, maxDepth, "", "")
        rt_1m_values[rt_1m_values == 0] = np.nan
        rt_1m_values = (rt_1m_values) ** (-1 / m)

        x_title = self.well.wellModel.get_label_for(self.phieCbo.currentText(), "Porosidad")
        ...
        for i in range(len(config_aux['rt_1m'])):
            config['line_groups'].append(
                {
                    'x_axis': config_aux['phie'][i],
                    'y_axis': config_aux['rt_1m'][i],
