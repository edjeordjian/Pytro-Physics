...
from PyQt6.QtWidgets import QLabel, QComboBox, QHBoxLayout, QVBoxLayout, QCheckBox, QLineEdit

from constants import MENU_CONSTANTS

from constants.messages_constants import MISSING_CURVES, INVALID_NUMERIC_INPUT

from constants.porosity_constants import (POROSITY_STYLE_LBL, DENSITY_CURVE, POROSITY_CURVE, TOTAL_POROSITY_BY_AG_NAME,
                                          ADJUST_RANGE_LBL, ADJUSTED_MIN_LBL, DEFAULT_ADJUSTED_MIN_LBL,
                                          ADJUSTED_MAX_LBL, DEFAULT_ADJUSTED_MAX_LBL, NEUTRON_CURVE)

from constants.tab_constants import TOTAL_POROSITY_TAB_NAME

from services.porosity_service import get_porosity_asquith_gibson
...
class PorosityAsquithGibson(QWidgetWithSections):
    def __init__(self):
        self.use_data_in_curve_selectors = False

        super().__init__(TOTAL_POROSITY_BY_AG_NAME)
    ...
    def initUI(self):
        ...
        self.add_serializable_attributes(self.curve_selectors + [self.depthFullLasRb, self.depthCustomRb,
                                         self.density_cbo, self.porosity_cbo, self.porosity_color, self.porosity_line,
    ...
    def density_curve_section(self):
        self.density_lbl = QLabel(DENSITY_CURVE)
    ...
    def porosity_curve_section(self):
        self.porosity_lbl = QLabel(NEUTRON_CURVE)

        self.porosity_cbo = QComboBox(self)

        self.porosity_cbo.setPlaceholderText(MENU_CONSTANTS.CURVE_CBO_PLACEHOLDER)

        self.curve_selectors \
            .append(self.porosity_cbo)

        self.porosity_layout = QHBoxLayout()

        self.porosity_layout \
            .addWidget(self.porosity_lbl)

        self.porosity_layout \
            .addWidget(self.porosity_cbo)

        self.add_layout_to_layout(self.porosity_layout,
                                  column=0)
    ...
    def preview(self):
        ...
        porosity = self.porosity_cbo \
            .currentText()

        if density is None or porosity is None:
        ...
        if self.x_adjusted_cb.isChecked():
            self.curve_to_save = get_porosity_asquith_gibson(density_curve_data,
                                                             porosity_curve_data,
                                                             forced_min,
                                                             forced_max)
        else:
        ...
        total_porosity_config = {
            'tab_name': TOTAL_POROSITY_BY_AG_NAME,

            'track_name': TOTAL_POROSITY_BY_AG_NAME,

            'curve_name': TOTAL_POROSITY_BY_AG_NAME,
        ...
        self.well \
            .graphicWindow \
            .draw_tracks(TOTAL_POROSITY_BY_AG_NAME)
