...
from constants.porosity_constants import (DELTA_T_MATRIX_CURVE, POROSITY_STYLE_LBL, DT_LOG_CURVE_LBL,
                                          DT_FLOW_CURVE_LBL, TOTAL_POROSITY_BY_WYLLIE_NAME, BCP_CURVE_LBL,
                                          POROSITY_CURVE, ADJUST_RANGE_LBL, ADJUSTED_MIN_LBL, DEFAULT_ADJUSTED_MIN_LBL,
                                          ADJUSTED_MAX_LBL, DEFAULT_ADJUSTED_MAX_LBL)

from constants.tab_constants import TOTAL_POROSITY_TAB_NAME

from services.porosity_service import get_porosity_by_wyllie

from services.tools.string_service import is_positive_number, are_numbers
...
class PorosityByWyllie(QWidgetWithSections):
    def __init__(self):
        self.use_data_in_curve_selectors = False

        super().__init__(TOTAL_POROSITY_BY_WYLLIE_NAME)
    ...
    def initUI(self):
        ...
        self.numeric_inputs.extend([self.delta_t_matrix_constant_textbox, self.bcp_constant_textbox,
                                    self.dt_flow_constant_textbox, self.x_adjusted_min_textbox,
        ...
        self.add_serializable_attributes(self.curve_selectors + [self.depthFullLasRb, self.depthCustomRb,
                                         ...
                                         self.dt_flow_constant_textbox, self.porosity_color, self.porosity_line,
                                         self.porosity_marker, self.customMinDepthQle, self.customMaxDepthQle,
                                         self.bcp_constant_textbox, self.x_adjusted_min_textbox,
    ...
    def dt_flow_section(self):
        self.dt_flow_lbl = QLabel(DT_FLOW_CURVE_LBL)

        self.dt_flow_constant_textbox = QLineEdit()

        self.dt_flow_layout = QHBoxLayout()

        self.dt_flow_layout \
            .addWidget(self.dt_flow_lbl)

        self.dt_flow_layout \
            .addWidget(self.dt_flow_constant_textbox,
                       alignment=Qt.AlignmentFlag
                       .AlignRight)

        self.add_layout_to_layout(self.dt_flow_layout)

    def bcp_section(self):
        self.bcp_lbl = QLabel(BCP_CURVE_LBL)

        self.bcp_constant_textbox = QLineEdit()

        self.bcp_layout = QHBoxLayout()

        self.bcp_layout \
            .addWidget(self.bcp_lbl)

        self.bcp_layout \
            .addWidget(self.bcp_constant_textbox,
                       alignment=Qt.AlignmentFlag
                       .AlignRight)

        self.add_layout_to_layout(self.bcp_layout)
    ...
    def preview(self):
        ...
        dt_flow = self.dt_flow_constant_textbox \
            .text()

        bcp = self.bcp_constant_textbox \
            .text()
        ...
        if (dt_log is None or dt_matrix is None
                or len(dt_flow) == 0
                or len(bcp) == 0):
            return AlertWindow(f"{MISSING_CURVES_OR_VALUES} {TOTAL_POROSITY_TAB_NAME}")

        if not is_positive_number(bcp):
            return get_positive_value_error_alert(BCP_CURVE_LBL)

        if not is_positive_number(dt_flow):
            return get_positive_value_error_alert(DT_FLOW_CURVE_LBL)
        ...
        if self.x_adjusted_cb.isChecked():
            self.curve_to_save = get_porosity_by_wyllie(dt_log_data,
                                                        float(dt_flow),
                                                        float(bcp),
                                                        dt_matrix,
                                                        forced_min,
        ...
        total_porosity_config = {
            'tab_name': TOTAL_POROSITY_BY_WYLLIE_NAME,

            'track_name': TOTAL_POROSITY_BY_WYLLIE_NAME,

            'curve_name': TOTAL_POROSITY_BY_WYLLIE_NAME,
        ...
        self.well \
            .graphicWindow \
            .draw_tracks(TOTAL_POROSITY_BY_WYLLIE_NAME)
