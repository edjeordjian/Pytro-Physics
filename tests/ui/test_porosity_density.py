...
def test_porosity_density_view(qtbot):
    window = init_config(qtbot, current_path)

    graphic_window = get_graphic_window(window)

    load_view_to_window(qtbot, graphic_window, current_path, "porosity_density.view")

    assert 0.10242693845374186 == graphic_window.curve_tracks[0] \
               .configs[TOTAL_POROSITY_TAB_NAME]["x_axis"][-1]
